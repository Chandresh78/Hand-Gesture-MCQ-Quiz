...
while True:
    ...
    if qNo < qTotal:
        ...
        if hands:
            ...
            cursor = lmList[8][:2]  # Extract only x and y coordinates
            point1 = lmList[8][:2]
            point2 = lmList[12][:2]
            length, info = detector.findDistance(point1, point2)
            print(length)
            if length < 60:
    ...
    cv2.rectangle(img, (150, 600), (barValue, 650), (0, 250, 0), cv2.FILLED)
